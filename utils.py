"""This file contains the utility functions for the GM-CNN."""
...
def generate_elements(group, order):
    ...
    if group == "dihedral":
        ...
        raise ValueError(
            "Unsupported group type. Supported types: 'dihedral', 'cyclic'."
        )
...
def get_group_matrix(elements):
    ...
    mat = np.array(
        [
            [f"{inverses[k].product(elements[j])}" for j in range(len(elements))]
